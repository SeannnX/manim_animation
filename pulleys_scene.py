...
class PulleyLecture(Scene):
    ...
    def construct(self):
        title = Tex("动滑轮与定滑轮的对比与应用", font_size=60)
        subtitle = Tex("机械优势、受力特点与绳长变化", font_size=36).next_to(title, DOWN)
        ...
        fixed_title = Tex("定滑轮" if self.camera.frame_height > 6 else "定滑轮", font_size=40)
        movable_title = Tex("动滑轮", font_size=40)
        ...
        comparison = VGroup(
            Tex("定滑轮: 省力不省距离, 改变力的方向", font_size=34),
            Tex("动滑轮: 省力但需要更长绳长, 不改变力的方向", font_size=34),
            Tex("组合滑轮组可兼顾方向与省力", font_size=34),
    ...
    def create_fixed_pulley_diagram(self):
        ...
        load_label = Tex("重物 G", font_size=32).next_to(load, DOWN, buff=0.2)
        ...
        force_text = Tex("F", font_size=32, color=YELLOW).next_to(force_arrow, RIGHT * 0.3)
    ...
    def create_movable_pulley_diagram(self):
        ...
        ratio_text = Tex("理论机械效率: 1/2", font_size=32).next_to(pulley, UP, buff=0.2)
    ...
    def explain_fixed_pulley(self, group):
        ...
        text = Tex("轴固定不动", "\n输出力方向改变", "\n机械优势约为 1", font_size=30, tex_environment="flushleft")
    ...
    def explain_movable_pulley(self, group):
        arrow = Arrow(start=group.get_right() + RIGHT * 0.5, end=group.get_right(), color=YELLOW)
        text = Tex("轴随重物移动", "\n分担重力: F≈G/2", "\n需要牺牲绳长", font_size=30, tex_environment="flushleft")
        box = VGroup(arrow, text.arrange(DOWN, aligned_edge=LEFT))
        box.next_to(group, RIGHT, buff=0.6)
